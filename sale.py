...
#         return {'value': result}
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        # 'ids' is *not* guaranteed to be an iterable!
        if not hasattr(ids, "__iter__"):
            ids = [ids]

        for sale_obj in self.browse(cr, uid, ids, context=context):
            if (sale_obj.shipped or vals.get("shipped")) \
                    and (sale_obj.invoiced or vals.get("invoiced")) \
                    and (sale_obj.state != 'done' or vals.get("state") != "done"):
